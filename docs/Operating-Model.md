...
- The custom domain `mafrick-munene-advocates.com` is in the process of
...
- `mafrick-munene-advocates.com` is a CNAME → `lglenz.github.io.`
...
| `mafrick-munene-advocates.com` shows cert error | GitHub Pages certificate for the custom host still being issued.   | `site-health.yml` → `http-check` (custom-domain target, warn-only). |
...
4. **Custom domain DNS / TLS** — once
   `mafrick-munene-advocates.com` resolves and GitHub has issued a
